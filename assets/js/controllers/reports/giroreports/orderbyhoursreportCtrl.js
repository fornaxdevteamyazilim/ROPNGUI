...
'use strict';
app.controller('ordersbyhoursreportCtrl', ordersbyhoursreportCtrl);
function ordersbyhoursreportCtrl($scope, $filter, $modal, $log, Restangular, ngTableParams, SweetAlert, toaster, $window, $compile, $translate, Excel, $timeout, ngnotifyService, $rootScope, $element) {
    $rootScope.uService.EnterController("ordersbyhoursreportCtrl");
$scope.NewDate = $filter('date')(ngnotifyService.ServerTime(), 'yyyy-MM-dd');
    ...
    $scope.LoadData = function () {
        ...
        Restangular.all('order/reports/OrderCountsByPamentTypePivot').getList(
                {
                    PaymentTypeID: ($scope.PaymentTypeID == null) ? 'All' : $scope.PaymentTypeID,
                    StartDate: ($scope.DateFromDate) ? "" + $scope.DateFromDate + "" : "" + $scope.NewDate + "",
                    EndDate: ($scope.DateToDate) ? "" + $scope.DateToDate + "" : "" + $scope.NewDate + "",
                    OrderSourceID: ($scope.OrderSourceID == null) ? 'All' : $scope.OrderSourceID
                }
    ...
    ctrl.table = {
        ...
        config: {
            ...
            rows: ["DeliveryDate", "day"],
            cols: ["Hours"],
            aggregatorName: "Count",
            aggregator: count(intFormat)(["Count"]),
            vals: ["Count"],
    ...
    $scope.GetSource = function (data) {
        $scope.OrderSourceID = data;
    };
    $scope.PaymentType = function (data) {
    ...
    $scope.ShowReport = function (edit) {
        ...
        if (!ctrl.table.editMode) {
            ctrl.table.config.aggregator = count(intFormat)(["Count"]);
        }
    ...
    $scope.$on('$destroy', function () {
        $element.remove();
        $rootScope.uService.ExitController("ordersbyhoursreportCtrl");
    });
};
