'use strict';
app.controller('salesbysourceCtrl', salesbysourceCtrl);
function salesbysourceCtrl($scope, $log, $modal, $filter, SweetAlert, Restangular, ngTableParams, toaster, $window, $stateParams, $rootScope, $location, $timeout, $translate, Excel, userService, ngnotifyService, $element) {
    $rootScope.uService.EnterController("salesbysourceCtrl");
    $scope.SalesBySource = [];
    ...
    $scope.StoreTypeID = -1;
    $scope.LoadSalesBySource = function (FromValue) {
        $scope.isWaiting = true;
        Restangular.all('StoreReports/SalesBySource').getList(
            {
    ...
    $scope.exportToExcel = function (tableId) {
        ...
        downloadLink.attr('download', 'KanalBazliSatis.xls');
    ...
    $scope.$on('$destroy', function () {
        $element.remove();
        $rootScope.uService.ExitController("salesbysourceCtrl");
    });
};
