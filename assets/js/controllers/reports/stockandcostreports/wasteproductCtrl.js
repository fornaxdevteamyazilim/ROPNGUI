...
function wasteproductCtrl($scope, $modal, $filter, SweetAlert, Restangular,$translate, toaster, $window, $rootScope, $location, Excel, $timeout, NG_SETTING, userService, ngnotifyService, $element) {
    ...
    $scope.WasteProductList = [];
    $scope.LoadWasteProductList = function (FromValue) {
        $scope.isWaiting = true;
        Restangular.all('order/demoreports/Wastedproducts').getList(
            {
                ...
                $scope.totalProductCount = $scope.sumColumnJS(result, "ProductCount");
                angular.copy(result, $scope.WasteProductList);
    ...
    $scope.exportToExcel = function (tableId) {
        ...
        downloadLink.attr('download', 'UrunAtikRaporu.xls');
    ...
    $scope.Back = function () {
        ...
        $element.remove();
        $rootScope.uService.ExitController("wasteproductCtrl");
    });
};
