'use strict';
app.controller('productsalespochetteCtrl', productsalespochetteCtrl);
function productsalespochetteCtrl($scope, $log, $modal, $filter, SweetAlert, Restangular, $interval, ngTableParams, toaster, $window, Excel, $timeout, $stateParams, $rootScope, $location, $translate, ngnotifyService, $element) {
    $rootScope.uService.EnterController("productsalespochetteCtrl");
    ...
    $scope.GetPrototype = function (data) {
        ...
        $scope.selectedrototype = $filter('filter')($scope.productprototypes, { id: data });
    };
    ...
    $scope.exportToExcel = function (tableId) {
        $scope.exportHref = Excel.tableToExcel(tableId, 'UrunSatisListesi');
    ...
    $scope.Back = function () {
        ...
        $element.remove();
        $rootScope.uService.ExitController("productsalespochetteCtrl");
    });
};
