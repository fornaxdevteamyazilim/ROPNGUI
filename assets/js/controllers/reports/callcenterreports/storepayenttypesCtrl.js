'use strict';
app.controller('storepayenttypesCtrl', storepayenttypesCtrl);
function storepayenttypesCtrl($scope, $log, $modal, $filter, SweetAlert, Restangular, $interval, $translate, ngTableParams, toaster, $window, $stateParams, $location, Excel, $timeout, NG_SETTING, ngnotifyService, $rootScope, $element) {
            $rootScope.uService.EnterController("storepayenttypesCtrl");  
    if (!$scope.StartDate) {
    ...
    $scope.LoadDailyTotals = function (FromValue) {
        ...
        Restangular.all('order/reports/storepayenttypes').getList(
            {
    ...
    $scope.DailyTotalsExcel = function () {
        location.href = NG_SETTING.apiServiceBaseUri + '/api/order/reports/storepayenttypesxls?StartDate=' + $scope.StartDate + '&EndDate=' + $scope.EndDate;
    };
    ...
    //    downloadLink.attr('download', 'CM Restoran Bazlı Ödeme Dağılımı.xls');
    //    downloadLink[0].click();
    //};
        $scope.exportToExcel = function (tableId) {
        $scope.exportHref = Excel.tableToExcel(tableId, 'CM Restoran Bazlı Ödeme Dağılımı');
    ...
    $scope.Back = function () {
        ...
        $element.remove();
        $rootScope.uService.ExitController("storepayenttypesCtrl");
    });
};
