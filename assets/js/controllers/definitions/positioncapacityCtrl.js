app.controller('positioncapacityCtrl', positioncapacityCtrl);
function positioncapacityCtrl($scope, $log, $modal, $filter, SweetAlert, Restangular, ngTableParams, toaster, $window, $stateParams, $rootScope, $location, $translate, $element) {
    $rootScope.uService.EnterController("positioncapacityCtrl");
    var pc = this;
    ...
    $scope.saveData = function (data) {
        if (data.restangularized && data.id) {
            ...
            Restangular.restangularizeElement('', data, 'PositionCapacity')
    ...
    pc.tableParams = new ngTableParams({
    ...
    },
        {
            getData: function ($defer, params) {
                Restangular.all('PositionCapacity').getList({
                    pageNo: params.page(),
                    pageSize: params.count(),
                    sort: params.orderBy(),
                    //search: (pc.search) ? "Name  like '%" + pc.search + "%'" : ""
                }).then(function (items) {
    ...
    $scope.$on('$destroy', function () {
        ...
        deregistration1();
        $element.remove();
        $rootScope.uService.ExitController("positioncapacityCtrl");
    });
};
