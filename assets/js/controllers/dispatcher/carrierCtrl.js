app.controller('carrierCtrl', carrierCtrl);
function carrierCtrl($scope, $log, $modal, $filter, SweetAlert, Restangular, ngTableParams, toaster, $window, $stateParams, $rootScope, $location, $translate, $modalInstance, userService, ngnotifyService) {
    ...
    $scope.saveData = function () {
        if (this.item.restangularized) {
            ...
            Restangular.restangularizeElement('', this.item, 'carrier')
            this.item.StartDate = $scope.StartDate;
            this.item.EndDate = $scope.EndDate;
    ...
    $scope.isActivePasif = function (data) {
        if (data == null || data == false) {
            ...
            for (var i = 0; i < $scope.carrier.length; i++) {
                $scope.StartDate = $scope.carrier[i].StartDate;
            }
