...
function orderpaymentCtrl($scope, $log, $modal, $filter, $modalInstance, Order, Restangular,PaymentRestangular, ngTableParams, SweetAlert, toaster, $window, $rootScope, $location, $translate, userService, ngnotifyService) {
    ...
    $scope.CalcSumOfPayments = function () {
        ...
        for (var i = 0; i < $scope.order.payments.length; i++) {
            result += parseFloat(Math.round($scope.order.payments[i].Amount * 100) / 100);
        }
        return result;
    };
    ...
    $scope.NumberValue = function (value) {
        if ($scope.key == true) {
            $scope.currentPayment.Amount = $scope.CalcRequiredAmount() / $scope.currentPayment.Amount;
    ...
    $scope.CalcSumOpr = function (operation) {
        ...
        if (operation == "") {
            ...
            $scope.currentPayment.Amount = 0.00;
        }
    };
    ...
    $scope.NewPayment = function () {
        ...
        var np = { OrderID: $scope.order.id, OrderPersonID: $scope.order.persons[0].id, Amount: $scope.CalcRequiredAmount() };
    ...
    $scope.ShowPerson = function (orderPersonID) {
        var personArray = $filter('filter')($scope.order.persons, { id: orderPersonID });
    ...
    $scope.getBekoECRPayment = function () {
        if ($rootScope.user.Store.MemberID == "507632258556") {
            ...
            chrome.experimental.arcsp.sendJsonToPos(JSON.stringify($scope.salesSlip), function (pos_result) {
                var result = JSON.parse(pos_result);
                $scope.receiptId = parseInt(result.body['receipt id']);
                var itemList = [];
                var item1 = { "name": "GIDA", "quantity": 10000, "section": 1, "price": amount, "pluId": 0 };
                itemList.push(item1);
                var paymentObj = {
                    "opType": 7,
                    "fiscal": $scope.fiscalId,
                ...
                chrome.experimental.arcsp.sendJsonToPos(JSON.stringify(paymentObj),
                    function (pos_result) { }); // sendJsonToPos finish  
            });
        }
    };
    $scope.setBekoECRPayment = function (paymentType) {
        if ($rootScope.user.Store.MemberID == "507632258556") {
            ...
            chrome.experimental.arcsp.sendJsonToPos(JSON.stringify(lastPaymentObj), function (pos_result) { });
        }
    };
    $scope.cancelBekoECRPayment = function () {
        ...
        chrome.experimental.arcsp.sendJsonToPos(JSON.stringify(PaymentObj), function (pos_result) { });
    };
    //**********//********** // ********** // ********** // ********** // **********// BEKO //**********//********** // ********** // ********** // ********** //
