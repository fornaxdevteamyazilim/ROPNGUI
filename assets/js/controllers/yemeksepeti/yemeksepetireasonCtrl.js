app.controller('yemeksepetireasonCtrl', CarrierCtrl);
function CarrierCtrl($rootScope, $scope, $log, $modal, $filter, SweetAlert, Restangular, ngTableParams, toaster, $window, $stateParams, $location, $translate, $element) {
    $rootScope.uService.EnterController("CarrierCtrl");
    var cs = this;
    $scope.translate = function () {
        $scope.trCarrierid = $translate.instant('main.CARRIERID');
        $scope.trReasonName = $translate.instant('main.REASONNAME');
        ...
        $scope.trCultureCode = $translate.instant('main.CULTURECODE');
        $scope.trReasonId = $translate.instant('main.REASONID');
        $scope.trCultureId = $translate.instant('main.CULTUREID');
    ...
    },
 {
     getData: function ($defer, params) {
         Restangular.all('yemeksepetireason').getList({
             pageNo: params.page(),
    ...
    $scope.saveData = function (data) {
        if (data.restangularized && data.id) {
            ...
            Restangular.restangularizeElement('', data, 'yemeksepetireason')
    ...
    $scope.$on('$destroy', function () {
        ...
        deregistration1();
        $element.remove();
        $rootScope.uService.ExitController("CarrierCtrl");
    });
};
